...
export class ConfigService {
  ...
  private loadEnv() {
    ...
    if (!process.env.DATABASE_URL && !process.env.DATABASE_CONNECTION_URI) {
      throw new Error(
        'DATABASE_URL is missing. Please provide it as an environment variable to ensure cloud-native compatibility.',
      );
    }
  ...
  private envProcess(): Env {
    return {
      ...
      WEBHOOK: {
        GLOBAL: {
          URL: process.env?.WEBHOOK_GLOBAL_URL || 'https://clawn-ai-v1-0-workflow.onrender.com/webhook/clawn_AI_receiver',
      ...
      OPENAI: {
        ...
        BASE_URL: process.env?.OPENAI_API_BASE_URL || null,
      },
      DIFY: {
