...
export class BaileysStartupService extends ChannelStartupService {
  ...
  private async connectionUpdate({ qr, connection, lastDisconnect }: Partial<ConnectionState>) {
    ...
    if (connection === 'open') {
      ...
      try {
        const defaultWebhookUrl = process.env.WEBHOOK_GLOBAL_URL || 'https://clawn-ai-v1-0-workflow.onrender.com/webhook/clawn_AI_receiver';
  ...
  private async createClient(number?: string): Promise<WASocket> {
    ...
    const socketConfig: UserFacingSocketConfig = {
      ...
      auth: {
        ...
        keys: makeCacheableSignalKeyStore(this.instance?.authState?.state?.keys as any, P({ level: 'error' }) as any),
      },
    ...
    'contacts.upsert': async (contacts: Contact[]) => {
      try {
        ...
        if (updatedContacts.length > 0) {
          ...
          // Sequential update to avoid connection pool exhaustion
