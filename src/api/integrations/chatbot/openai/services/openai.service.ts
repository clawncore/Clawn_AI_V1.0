...
export class OpenaiService extends BaseChatbotService<OpenaiBot, OpenaiSetting> {
  ...
  public async process(
    ...
    try {
      ...
      if (content.startsWith('audioMessage|') && msg) {
        ...
        // Use creds key if available, otherwise fallback to global env key
        const apiKey = creds?.apiKey && creds.apiKey.startsWith('sk-') 
          ? creds.apiKey 
          : this.configService.get<OpenaiConfig>('OPENAI').API_KEY_GLOBAL;

        if (!apiKey) {
          this.logger.error('No OpenAI/OpenRouter API key found in credentials or global config.');
          return;
        }
  ...
  protected async sendMessageToBot(
    ...
    if (!this.client) {
      ...
      if (!apiKey) {
        this.logger.error(`No OpenAI/OpenRouter API key found in sendMessageToBot.`);
        return;
      }
  ...
  private async processChatCompletionMessage(
    ...
    if (!this.client) {
      ...
      if (!apiKey) {
        this.logger.error(`No OpenAI/OpenRouter API key found in processChatCompletionMessage.`);
        return 'Error: OpenAI/OpenRouter API key not found';
      }
